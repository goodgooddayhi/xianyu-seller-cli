...
## 💡 为什么选择 xianyu-seller-cli？

### 对比其他方案

| 方案 | 风控风险 | 说明 |
|------|---------|------|
| **直接调 API** | 🔴 高 | 绕过前端，极易被检测，封号率高 |
| **无头浏览器** | 🟡 中 | 缺少真实浏览器指纹，容易被风控 |
| **RPA 模拟点击** | 🟡 中 | 操作轨迹不自然，容易触发验证 |
| **xianyu-seller-cli** | 🟢 低 | 基于官方客户端，行为与真人一致 |

### 核心优势

1. **基于官方客户端** — 闲鱼卖家客服 Electron 应用，所有操作在真实浏览器中执行，与手动操作完全一致

2. **风控友好** — 通过 CDP 协议操控已登录的浏览器会话，不会产生异常登录、异地访问等风控信号

3. **会话状态完整** — 复用客户端的 Cookie 和登录态，无需额外处理认证，避免频繁登录触发风控

4. **操作行为自然** — DOM 操作等同于用户点击，没有脚本痕迹，不会被反爬虫系统识别

5. **本地运行** — 所有操作在本地执行，数据不经过第三方服务器，安全可控
...
本项目仅供学习与研究使用。使用本工具所产生的一切行为及后果由用户自行承担。

- 本项目与闲鱼官方无关，非闲鱼官方产品
- 使用本工具请遵守闲鱼社区规范和相关法律法规
- 因使用本工具导致的账号封禁、数据丢失等问题，作者不承担任何责任
- 请勿将本工具用于任何违法违规用途
- 建议在测试账号上充分验证后再用于生产环境
